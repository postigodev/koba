...
pub struct WorkingTreeAnalysis {
    ...
    pub check_recommendations: Vec<CheckRecommendation>,
    pub risks: Vec<Risk>,
    pub primary_plan: Option<CommitPlan>,
    pub is_clean: bool,
}
...
impl CommitPlan {
    pub fn git_add_command(&self) -> String {
        let paths = self.files.iter().map(String::as_str).collect::<Vec<_>>();
        format!("git add -- {}", quote_paths(&paths))
    }

    pub fn git_commit_command(&self) -> String {
        format!("git commit -m {}", quote_shell_arg(&self.message))
    }
}
...
pub fn analyze_cwd(cwd: &Path) -> Result<WorkingTreeAnalysis, String> {
    let files = git_status::status_entries(cwd)?;
    Ok(analyze(cwd, files))
}

pub fn analyze(cwd: &Path, files: Vec<WorkingTreeFile>) -> WorkingTreeAnalysis {
    ...
    let commit_plans = plan_commits(&files);
    let check_recommendations = recommend_checks(cwd, &files);
    let risks = assess_risks(&files, &commit_plans);
    let primary_plan = select_primary_plan(&commit_plans);
...
fn plan_for_group(concept: ChangeConcept, files: Vec<&WorkingTreeFile>) -> CommitPlan {
    ...
    let (message, reason) = match concept {
        ...
        ChangeConcept::Rust
            if paths
                .iter()
                .all(|path| path_classification::is_test_file(path)) =>
        {
            ("test(cli): add coverage", "matched Rust test files")
        }
        ChangeConcept::Rust => (
        ...
        ChangeConcept::Docs
            if paths
                .iter()
                .any(|path| path_classification::normalize(path).contains("docs/product")) =>
        {
            (
                "docs(product): update product documentation",
                "matched product documentation files",
            )
        }
...
fn select_primary_plan(plans: &[CommitPlan]) -> Option<CommitPlan> {
    plans
        .iter()
        .max_by_key(|plan| (plan.confidence, plan.files.len()))
        .cloned()
}

fn recommend_checks(cwd: &Path, files: &[WorkingTreeFile]) -> Vec<CheckRecommendation> {
    if files.is_empty() {
        return Vec::new();
    }
    ...
    if has_scoop {
        ...
        {
            checks.push(CheckRecommendation {
                command: format!("python -m json.tool {}", quote_shell_arg(&file.path)),
...
fn node_script_checks(cwd: &Path) -> Vec<CheckRecommendation> {
...
fn python_checks(cwd: &Path) -> Vec<CheckRecommendation> {
...
fn assess_risks(files: &[WorkingTreeFile], plans: &[CommitPlan]) -> Vec<Risk> {
    ...
    if plans.len() > 1 {
        ...
        risks.push(Risk {
            status: Status::Ok,
            message: "working tree appears coherent".to_owned(),
        });
    }
...
pub fn quote_paths(paths: &[&str]) -> String {
    paths
        .iter()
...
mod tests {
    ...
    fn file_status(status: &str, path: &str) -> WorkingTreeFile {
        WorkingTreeFile::from_status(status, path)
    }
    ...
    fn parses_status_counts_without_mutation() {
        ...
        let report = analyze(Path::new("."), files);
    ...
    fn clean_tree_has_no_commit_plan() {
        ...
        let report = analyze_cwd(fixture.path()).unwrap();

        assert!(report.is_clean);
        assert!(report.commit_plans.is_empty());
        assert!(report.primary_plan.is_none());
        assert!(report
            .risks
            .iter()
            .any(|risk| risk.message == "working tree is clean"));
    }
    ...
    fn docs_only_readme_and_agent_docs_stay_docs_agents() {
        let report = analyze(
            Path::new("."),
            vec![file("README.md"), file("docs/agents.md")],
        );

        assert_eq!(report.commit_plans.len(), 1);
        assert_eq!(
            report.primary_plan.unwrap().message,
            "docs(agents): update agent documentation"
        );
        assert!(report
            .check_recommendations
    ...
    fn analysis_refactor_groups_shared_modules_and_consumers_together() {
        ...
        assert!(report
            .risks
            .iter()
            .any(|risk| risk.status == Status::Ok && risk.message.contains("coherent")));
    }

    #[test]
    fn isolated_changes_module_keeps_changes_scope() {
        let report = analyze(Path::new("."), vec![file("crates/koba/src/changes.rs")]);
    ...
    fn scoop_manifest_recommends_packaging_and_json_hash_review() {
        let report = analyze(
            Path::new("."),
            vec![file("packaging/scoop/bucket/koba.json")],
        );
    ...
    fn github_workflow_recommends_ci_review() {
        let report = analyze(Path::new("."), vec![file(".github/workflows/ci.yml")]);
    ...
    fn rust_source_recommends_koba_workspace_checks_when_detected() {
        let cwd = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            ...
            .unwrap()
            .to_path_buf();
        let report = analyze(&cwd, vec![file("crates/koba/src/output.rs")]);
    ...
    fn mixed_rust_and_readme_warns_when_grouped_together() {
        let report = analyze(
            Path::new("."),
            vec![file("crates/koba/src/output.rs"), file("README.md")],
        );
    ...
    #[test]
    fn untracked_files_are_kept_in_analysis_and_primary_plan() {
        let files = git_status::parse_porcelain_z(
        ...
        let displayed = report
            ...
            .collect::<Vec<_>>();
        let planned = report.primary_plan.unwrap().files;

        assert!(displayed.contains(&"crates/koba/src/git_status.rs"));
        assert!(displayed.contains(&"crates/koba/src/path_classification.rs"));
        assert!(planned.contains(&"crates/koba/src/git_status.rs".to_owned()));
        assert!(planned.contains(&"crates/koba/src/path_classification.rs".to_owned()));
    }

    #[test]
    fn staged_unstaged_and_untracked_counts_are_stable() {
        let report = analyze(
            Path::new("."),
            vec![
                file_status("M ", "staged.rs"),
                file_status(" M", "unstaged.rs"),
                file_status("??", "new.rs"),
            ],
        );
    ...
    impl TempTree {
        fn new() -> Self {
            let id = SystemTime::now()
                ...
                .unwrap()
                .as_nanos();
            let path = std::env::temp_dir().join(format!("koba-analysis-test-{id}"));
